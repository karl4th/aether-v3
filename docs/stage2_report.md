...
| Phase 2A | Frozen Qwen3-4B, ratio 1 transcription | **Complete; selected** |
| Phase 2B | Frozen Qwen3-4B, ratio 4 transcription | **Stopped as negative result** |
| Phase 3 | Qwen LoRA | Deferred; not required for the current pass |
| Phase 4 | Selective AetherSpeech unfreezing | Deferred; not required for the current pass |
...
## 20. Full frozen-R1 outcome

The accepted full run is `run260922-221648`. It trained the R1 Connector on
all 132,553 `train-clean-100 + train-clean-360` examples at effective batch
16 while keeping AetherSpeech and Qwen3-4B frozen. The initial schedule
reached a validation plateau at step 10,000. A bounded recovery from the
best-WER checkpoint used a shorter, lower-learning-rate schedule; the final
selected `best_wer.pt` records step 12,000.

The 256-example selection subset was useful for frequent evaluation but was
optimistic relative to the complete split. It reached approximately 15.27%
WER near the end of recovery. Final quality claims therefore use the complete
2,703-example validation split and 2,620-example test split below.

The full run establishes that a small trainable R1 Connector can translate
frozen AetherSpeech states into inputs that a frozen Qwen3-4B can decode as
coherent transcripts. LoRA and selective AetherSpeech unfreezing were not
needed to establish this property and are deferred.

## 21. Phase 7A full transcription evaluation

The selected step-12,000 checkpoint was evaluated with greedy generation on
every example in both splits:

| Split | Examples | Loss | WER | CER | Throughput |
|---|---:|---:|---:|---:|---:|
| Validation | 2,703 | `0.415699` | **17.29%** | **9.18%** | 6.66 utt/s |
| Test | 2,620 | `0.412301` | **16.83%** | **8.83%** | 8.74 utt/s |

Validation and test loss are nearly identical, and test WER is 0.45 absolute
percentage points lower than validation WER. The close independent-split
results provide no evidence of material overfitting. Errors in inspected
samples are localized phonetic substitutions, deletions, morphology, and
proper names rather than unconstrained language-model output.

The current Stage 1 CTC reference is 16.35% WER / 5.88% CER. On the reported
test split, the direct frozen-Qwen path is 0.48 absolute WER points and 2.95
absolute CER points worse. It therefore does not yet improve transcription
quality over Stage 1, but it does validate direct continuous speech input to
the LLM without an intermediate transcript.

## 22. Invalid first test cache and correction

The first test evaluation produced 524.93% WER / 563.01% CER with repetitive
multilingual generations. That result is invalid. During a recovery-session
model reconstruction, the Stage 1 weights were not reloaded into
`model.encoder`. Training and validation remained valid because both consumed
speech-state caches created earlier with the correct frozen Stage 1 encoder;
the newly built test cache alone was generated by a randomly initialized
encoder.

The invalid test cache was deleted. The real Stage 1 checkpoint was loaded
strictly, all 2,620 test speech states were rebuilt, and test evaluation was
repeated. The corrected 16.83% WER / 8.83% CER result in Section 21 supersedes
the invalid first result. The machine-readable corrected report is
`phase7a_corrected_test_eval.json` in the run directory.

This incident also establishes an inference-packaging requirement: any
portable loader must restore and verify Mimi, Stage 1 AetherSpeech, Connector,
